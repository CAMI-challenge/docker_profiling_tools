		...
		open (IN, $ENV{$listing}) || die "file '".$ENV{$listing}."' not found: $!";
			while (my $line = <IN>) {
				...
				if (-e $line) {
					...
					my $commonHead = "rm -rf /tmp/motus.*";
					$commonHead .= " && cd /tmp/";
					$commonHead .= " && perl ".$ENV{PREFIX}."/src/".$ENV{TOOLNAME}."/mOTUs.pl --processors=".$ENV{NCORES}." ".$line;
					$commonHead .= " ; resfile=`find /tmp/ -name \"*insert.mm.dist.among.unique.scaled.taxaid.gz\"`";
					$commonHead .= " && zcat \$resfile > $resultfilename.orig";
